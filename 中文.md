...
# 预览
我有一个个人工具包（https://github.com/bbqz007/KTL）可以预览 MicroMsg 数据库。当你获取到你的十六进制密钥并想查看你的数据库时，它可以帮助你。我的工具包不链接任何 ws2_32.dll 或与 ws2_32.dll 相关的任何 DLL。我的工具包有一个 jitC，可以作为插件运行 C++ 代码。我为设置十六进制密钥和其他加密内容编写了一个补丁 AlgoDataTool.cpp。你可以根据需要修改它并决定是否运行。我的工具包主要用于离线分析股市，数据源包括类似 sqlite3 的数据库。
